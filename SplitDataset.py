...
class SplitDataset(ABC):
    ...
    def _training_validation_split(self, questions, snippets, labels):
        """
        ...
        if (self._val_set_size > 0):
            ...
            self._train_X1, self._val_X1, self._train_X2, self._val_X2, self._train_Y, self._val_Y = sklearn.model_selection.train_test_split(
                questions, snippets, labels, test_size=self._val_set_size, random_state=self.seed)
        else:
            # set the data to unsplit
            self._train_X1 = questions
            self._train_X2 = snippets
    ...
    def get_train_data(self):
        if self._train_X1 is None or self._train_Y is None or self._train_X2 is None:
            raise Exception(
                "Error: train data does not exist, you must call _training_validation_split after loading data")
        return self._train_X1, self._train_Y, self._train_X2

    def get_validation_data(self):
        if self._val_X1 is None or self._val_Y is None or self._val_X2 is None:
            raise Exception("Error: val data does not exist, you must specify a validation split percent")
        return self._val_X1, self._val_Y, self._val_X2
...
class Binary_Text_Classification_Dataset(SplitDataset):
    '''
    ...
    def __init__(self, data_file_path, snippet_column_name=None, question_column_name=None, label_column_name=None,
                 label_column_names=None, seed=SEED, validation_set_size=0):
        '''
        ...
        '''
        SplitDataset.__init__(self, seed=seed, validation_set_size=validation_set_size)
        ...
        if (
            ...
            if (question_column_name is None or label_column_name is None or snippet_column_name is None):
                question_column_name = 'question'
                snippet_column_name = 'snippet'
                label_column_name = 'label'
                df = pd.read_csv(data_file_path, header=None,
                                 names=[question_column_name, snippet_column_name, label_column_name],
                                 delimiter='\t').dropna()
            else:
        ...
        # load multilabel classification data. Column names are required
            ...
            if (question_column_name is None):
        ...
        # load the data
        raw_data = df[question_column_name]
        questions = df[question_column_name].values.tolist()
        snippets = df[snippet_column_name].values.tolist()
        ...
        self._training_validation_split(questions, snippets, labels)
